...
[source,text]
----
  win-pc01 (netflowAgent)  ──IPFIX UDP :9995──►  nfcapd → live/win-pc01/
...
| Сбор трафика, экспорт IPFIX (рекомендуется) или NetFlow v9 по UDP
...
Агент link:netflowAgent-design.adoc[netflowAgent] на каждом хосте отправляет IPFIX (рекомендуется для v0.3.1+) или NetFlow v9 на коллектор. IP сервера — из шага 1 (`ip -4 addr`).
