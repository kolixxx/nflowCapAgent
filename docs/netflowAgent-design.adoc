...
| **v0.2.0**
| **стабильная** (Windows win-pc01)
| Npcap + exe + config + `install-service.ps1`; служба, лог, reboot OK (принято 2026-07-06)
| тег `v0.2.0`, ветка `main`

|===

[IMPORTANT]
====
*Windows endpoint принят.* Установка вручную (~3 шага): Npcap → скопировать файлы → `install-service.ps1`. Отдельный setup.exe *не планируется* — см. <<решение-без-установщика-windows>>.

Откат на *v0.1.1* (только ручной cmd): тег `v0.1.1`.
====
...
[source,bash]
----
# стабильная Windows v0.2.0
git clone --depth 1 --branch v0.2.0 https://github.com/kolixxx/nflowCapAgent.git

# откат v0.1.1 (ручной запуск)
...
==== 2b. Linux endpoint (далее)

* Linux: сборка агента, systemd unit
* `install.sh` для Ubuntu endpoint → `:9996` / `ubuntu-pc01`
* Документация: link:netflowAgent-install-linux.adoc[install-linux]

*Тестер:* второй источник в nfsen-ng, график `ubuntu-pc01`.

[[этап-3]]
=== Этап 3 — IPFIX и расширение

*После* этапа 2b (Linux agent на `ubuntu-pc01` работает в проде).

* IPFIX export (`format = "ipfix"` в config; nfcapd уже принимает IPFIX)
* IPv6 flows
...
[NOTE]
====
*Когда IPFIX:* не раньше Linux endpoint. Сначала два стабильных источника (Windows + Linux) на NetFlow v9, затем IPFIX как улучшение протокола без смены коллектора.
====
