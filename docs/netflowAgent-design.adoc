...
[[agent-log]]
== Лог netflowAgent (консоль)

При запуске `netflowAgent.exe --config config.toml` агент пишет события в **stdout** (окно cmd/PowerShell). Уровень задаётся `logging.level` в config (по умолчанию `info`).
