...
| Npcap + служба + ps1; принято 2026-07-06
| тег `v0.2.0`

| **v0.2.1**
| **тест** (Linux ubuntu-pc01)
| libpcap + systemd + `build-linux.sh` / `install-linux.sh`; тест Ubuntu 18.04
| тег `v0.2.1-linux-test`, ветка `main`
...
==== 2b. Linux endpoint — v0.2.1 (тест на Ubuntu 18.04)
...
* [x] `interface = auto` — пропуск `lo`

*Тестер:* Ubuntu 18.04, `./build-linux.sh` → `install-linux.sh`, график `ubuntu-pc01` в nfsen-ng.
...
| 5
| AI
| ~~Этап 2b: Linux agent + systemd~~ ✅ v0.2.1 (код)
| build-linux.sh, install-linux.sh

| 6
| Вы
| Тест Ubuntu 18.04 → `:9996`, график `ubuntu-pc01`
