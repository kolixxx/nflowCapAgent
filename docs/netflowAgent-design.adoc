...
=== Совместимость с OpenSearch

Напрямую отправлять данные в OpenSearch агент не может. `netflowAgent` передаёт бинарные NetFlow v9/IPFIX datagram по UDP, а OpenSearch принимает документы через HTTP API.

Для OpenSearch нужен промежуточный декодер:

[source,text]
----
netflowAgent → IPFIX/UDP → IPFIX collector/decoder → JSON → OpenSearch
----

В роли декодера можно использовать совместимый Logstash pipeline, ElastiFlow, `nfcapd`/`nfdump` с отдельным JSON-exporter или собственный сервис. Поддержку IPFIX и OpenSearch необходимо проверять для конкретной версии выбранного продукта.

Сам агент менять не требуется, если промежуточный коллектор принимает стандартные IPFIX templates и Information Elements. Указывать адрес OpenSearch напрямую в `collector.host` нельзя.
...
| Старт; поля `target` (IP:port коллектора), `export=ipfix`, `interface`, `version`
...
| Агент отправил на коллектор IPFIX/NetFlow v9 datagram с **завершёнными** flow
...
[export]
format = "ipfix"         # рекомендуется для v0.3.1+
observation_domain_id = 1
source_id = 1

[capture]
interface = "\\Device\\NPF_{GUID}"  # значение из --list-devices
promiscuous = false
