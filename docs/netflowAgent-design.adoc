...
=== Этап 1.5 — полировка v0.1.1 (следующий)

* [ ] NetFlow v9: убрать `Event: INVALID` в nfdump (шаблон полей)
* [ ] Логи в консоли Windows без ANSI-«крякозябр» (`set NO_COLOR=1` или `with_ansi(false)`)
* [ ] `config.example.toml`: `interface = "auto"` по умолчанию
* [ ] Сборка → `agent/dist/win-x64/` → push на GitHub

*Тестер:* заменить exe на VM, прогнать 30 мин, убедиться что flows в nfdump без INVALID (или меньше).

[[этап-2]]
=== Этап 2 — сервис и установка

* Windows Service (автозапуск, без окна cmd)
* Логи в файл (`config.toml` → `logging.file`)
...
| 1
| AI
| Этап 1.5: v0.1.1 (INVALID, логи, config)
| Новый `netflowAgent.exe` на GitHub

| 2
| Вы
| Тест v0.1.1 на Win Server 2019 (~30–60 мин)
| OK / список замечаний

| 3
| AI
| Этап 2: Windows Service + лог в файл
| exe + инструкция «установка службы»

| 4
| Вы
| Тест службы: reboot, автозапуск
| Служба работает без cmd

| 5
| AI
| Этап 2: установщик Windows (Npcap)
| setup.exe или скрипт установки

| 6
| AI
| Этап 2: Linux agent + systemd
