...
| `format = "ipfix"` или `netflow9` (template 258); оба endpoint
| тег `v0.3.0`
...
[IMPORTANT]
====
*v0.3.0* — IPFIX + TCP flags на **обоих** endpoint. Откат без flags: тег `v0.2.2`.

* `format = "netflow9"` — NetFlow v9 template 258 (+ TCP_FLAGS)
* `format = "ipfix"` — IPFIX (nfcapd принимает оба)
...
[source,bash]
----
# v0.3.0 (IPFIX + TCP flags)
git clone --depth 1 --branch v0.3.0 https://github.com/kolixxx/nflowCapAgent.git
...
[[этап-3]]
=== Этап 3 — IPFIX и TCP flags — v0.3.0
...
Агент **до v0.3.0 не собирал** TCP flags — только 5-tuple, байты и пакеты.
