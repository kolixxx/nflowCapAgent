...
| **v0.2.0**
| стабильная (Windows only)
| Служба + ps1; суперседед **v0.2.2**
| тег `v0.2.0`

| **v0.2.2**
| **стабильная** (оба endpoint)
| NetFlow v9: Windows `win-pc01` :9995 + Linux `ubuntu-pc01` :9996; службы, reboot OK
| тег `v0.2.2`, link:https://github.com/kolixxx/nflowCapAgent/releases/tag/v0.2.2[Release]
...
[IMPORTANT]
====
*Релиз v0.2.2* — NetFlow v9 полностью рабочий на **обоих** endpoint-хостах (принято 2026-07-06).

* Windows: Npcap → `install-service.ps1` — link:netflowAgent-install-windows.adoc[install-windows]
* Linux: libpcap → `build-linux.sh` → `install-linux.sh` — link:netflowAgent-install-linux.adoc[install-linux]

Setup.exe не планируется. Откат Windows без службы: тег `v0.1.1`.
====
...
*Следующий этап.* NetFlow v9 на обоих endpoint принят в <<versions,v0.2.2>>.
...
[NOTE]
====
*Когда IPFIX:* этап 2b завершён (v0.2.2). IPFIX — улучшение протокола без смены коллектора.
====
